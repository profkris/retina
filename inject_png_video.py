...
def conc_movie(graph=None, data=None, time=None, recon_times=None, odir=None, win_width=1920, win_height=1080, quick=False, skip=1,log_scale=False, movie_naming=True, cmap_range=[0, 0.01],make_mp4=True, frame_size=(321, 100), pad_to_square=True,eye=None, output_mesh=False):

    if data is None:
        raise ValueError("data must be provided for headless conc_movie export")
    ...
    frame_dir = os.path.join(odir, "frames")
    os.makedirs(frame_dir, exist_ok=True)

    frame_paths = []
    print(f"Saving {len(recon_time_ind)} PNG frames...")
    ...
    for count, i in enumerate(tqdm(recon_time_ind)):
        ...
        # Pad if necessary
        if pad_to_square:
            padded = np.zeros(frame_size[0] * frame_size[0])
            padded[:conc.size] = conc
            image = padded.reshape((frame_size[0], frame_size[0]))
        else:
            padded = np.zeros(np.prod(frame_size))
            padded[:conc.size] = conc
            image = padded.reshape(frame_size)

        # Save PNG
        fname = f"frame_{count:04d}.png" if movie_naming else f"{i}.png"
        fpath = os.path.join(frame_dir, fname)

        fig = plt.figure(figsize=(6, 6), dpi=100)
        ax = fig.add_axes([0, 0, 1, 1])
        ax.imshow(image, cmap='gray', vmin=cmap_range[0], vmax=cmap_range[1])
        ax.axis('off')
    ...
    print(f"✅ PNG images saved to: {frame_dir}")

    # Optional: Create MP4 using matplotlib
    if make_mp4:
        print("Creating MP4 video...")
        video_path = os.path.join(odir, "concentration_video.mp4")
        fig = plt.figure(figsize=(6, 6), dpi=100)
        ax = fig.add_axes([0, 0, 1, 1])
        im = ax.imshow(np.zeros((frame_size[0], frame_size[0])), cmap='gray', vmin=cmap_range[0], vmax=cmap_range[1])
        ax.axis('off')

        writer = FFMpegWriter(fps=30, metadata=dict(artist='AutoSim'), bitrate=1800)
