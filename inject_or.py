...
def conc_movie(graph, data=None, time=None, recon_times=None, odir=None,
               ...
               eye=None, highlight_optic_disc=False, highlight_macula=False, movie_naming=True,
               cmap_range=[0, 0.01], output_mesh=False):
    ...
    for i in tqdm(recon_time_ind):
        ...
        if cname not in scalarNames and data is None:
            ...
            if tp is None:
                bgcolor = arr([0., 0., 0.])
                tp = graph.plot_graph(show=False, block=False, bgcolor=bgcolor, min_radius=min_radius,
                                      domain_type=domain_type, domain=domain,
            ...
            if data is not None:
                ...
                tp.set_cylinder_colors(edge_color=cur_conc_edge, cmap='gray',
                                       cmap_range=cmap_range, scalar_color_name='')
            else:
            ...
            if output_mesh:
                ...
                if count == 0:
                    ...
                    mesh_vertex_colors = np.zeros([recon_time_ind.shape[0],
                                                   mesh_vertex_colors_cur.shape[0], 3])
                mesh_vertex_colors_cur = np.asarray(gmesh.vertex_colors)
                ...
                if highlight_optic_disc and eye is not None:
                    ...
                    centre[2] = -20.
                    tp.add_torus(color=[1., 1., 1.], centre=centre,
                                 torus_radius=eye.optic_disc_radius / 2., tube_radius=20.)
                if highlight_macula and eye is not None:
                    centre = eye.fovea_centre
                    centre[2] = -20.
                    tp.add_cylinder(color=[0., 0., 0.], centre=centre,
                                    radius=eye.fovea_radius, height=20.)

                tp.update()

                # ✅ HEADLESS RENDERING SUPPORT
                if tp.headless:
                    img_np = tp.get_numpy_image()
                    Image.fromarray(img_np).save(grab_file)
                else:
                    tp.screen_grab(grab_file)

                if not os.path.exists(grab_file):
                    print(f"❌ Failed to write PNG: {grab_file}")
                else:
                    print(f"✅ PNG successfully written: {grab_file}")
    ...
    for i in tqdm(recon_time_ind): #range(0,nt,skip):
        ...
        if cname not in scalarNames and data is None:
            ...
            if output_mesh:
                ...
                tp.update()
                tp.screen_grab(grab_file) #------------- Updated Version----------------#
                if not os.path.exists(grab_file):
                    print(f"❌ Failed to write PNG: {grab_file}")
                else:
                    print(f"✅ PNG successfully written: {grab_file}")
...
def conc_movie(graph, data=None, time=None, recon_times=None, odir=None,
    ...
    tp = None
    domain_type = 'cylinder'
    domain = eye.domain if eye is not None else np.array([[-20000, 20000], [-20000, 20000], [-10000, 10000]])

    # 🔵 Initialize video writer
    video_path = os.path.join(odir, 'concentration.mp4')
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video_writer = cv2.VideoWriter(video_path, fourcc, 10, (win_width, win_height))
    ...
    for i in tqdm(recon_time_ind):
        ...
        if tp is None:
            ...
            tp = graph.plot_graph(show=False, block=False, bgcolor=bgcolor,
                                  min_radius=min_radius, domain_type=domain_type,
                                  domain=domain, win_width=win_width, win_height=win_height)
        ...
        if data is not None:
            ...
            tp.set_cylinder_colors(scalar_color_name=cname, cmap='gray',
        ...
        if output_mesh:
            ...
            import open3d as o3d
            if count == 0:
                mesh_file = join(odir, f'mesh_0000.ply')
                o3d.io.write_triangle_mesh(mesh_file, gmesh, write_vertex_colors=True)
                mesh_vertex_colors_cur = np.asarray(gmesh.vertex_colors)
                mesh_vertex_colors = np.zeros([recon_time_ind.shape[0], mesh_vertex_colors_cur.shape[0], 3])
            mesh_vertex_colors_cur = np.asarray(gmesh.vertex_colors)
            ...
            # 🔵 New: Get frame image directly from tp
            #img = tp.get_numpy_image()  # You must ensure this method is defined
            img = tp.vis.screenshot(return_img=True)
            ...
            if img is not None:
                ...
                print(f"Frame {fid} written to video.")
            else:
                print(f"[WARNING] Frame {fid} skipped: get_numpy_image() returned None.")

        count += 1

    if output_mesh:
        ofile = join(odir, 'vertex_colors.npy')
...
def conc_movie(graph, data=None, time=None, recon_times=None, odir=None,
    ...
    if recon_times is not None:
        ...
        recon_time_ind = np.arange(nt)

    min_radius = 0.
    bgcolor = np.array([0., 0., 0.])
    domain = eye.domain if eye else np.array([[-20000, 20000], [-20000, 20000], [-10000, 10000]])
    domain_type = 'cylinder'
    ...
    # Set up video writer
    fps = 10
    video_path = join(odir, "concentration_simulation.mp4")
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video_writer = cv2.VideoWriter(video_path, fourcc, fps, (win_width, win_height))

    print(f"Recording video to: {video_path}")
    for count, i in enumerate(tqdm(recon_time_ind)):
        cname = f'Concentration_{str(i).zfill(8)}'
        ...
        if data is not None:
            ...
            if log_scale:
                cur_conc_edge[cur_conc_edge <= 0.] = 1e-12
                cur_conc_edge = np.log(cur_conc_edge)
            tp.set_cylinder_colors(edge_color=cur_conc_edge, cmap='gray', cmap_range=cmap_range)
        else:
            tp.set_cylinder_colors(scalar_color_name=cname, cmap='gray', cmap_range=cmap_range)

        # Highlight regions if requested
        if highlight_optic_disc and eye:
            centre = eye.optic_disc_centre.copy()
            centre[2] = -20.
            tp.add_torus(color=[1., 1., 1.], centre=centre,
                         torus_radius=eye.optic_disc_radius / 2., tube_radius=20.)
        if highlight_macula and eye:
            centre = eye.fovea_centre.copy()
            centre[2] = -20.
            tp.add_cylinder(color=[0., 0., 0.], centre=centre,
                            radius=eye.fovea_radius, height=20.)
        ...
        # Grab frame as image array and write to video
        frame = tp.screen_grab()
        if frame is not None:
            frame = (np.asarray(frame) * 255).astype(np.uint8)
            frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
            video_writer.write(frame)
        else:
            print("[WARNING] Empty frame at timestamp {i}")
...
def conc_movie(graph, data=None, time=None, recon_times=None, odir=None,
               win_width=1920, win_height=1080, quick=False, skip=1,
               log_scale=False, eye=None, highlight_optic_disc=False,
               highlight_macula=False, movie_naming=True,
               cmap_range=[0, 0.01], output_mesh=False):
    ...
    tp = None
    domain_type = 'cylinder'
    domain = eye.domain if eye is not None else arr([[-20000, 20000], [-20000, 20000], [-10000, 10000]])
    min_radius = 0.
    ...
    for i in tqdm(recon_time_ind):
        ...
        if output_mesh:
            ...
            if count == 0:
                ...
                o3d.io.write_triangle_mesh(mesh_file, gmesh, write_vertex_colors=True)
                mesh_vertex_colors_cur = np.asarray(gmesh.vertex_colors)
                mesh_vertex_colors = np.zeros([nrecon, mesh_vertex_colors_cur.shape[0], 3])
            ...
            tp.screen_grab(grab_file)
            print(f'[INFO] Saved image: {grab_file}')
        count += 1

    # === Save mesh vertex colors ===
    if output_mesh and mesh_vertex_colors is not None:
        for i in range(nrecon):
            np.save(join(odir, f'vertex_colors_{str(i).zfill(8)}.npy'), mesh_vertex_colors[i])
        print(f"[INFO] Saved vertex colors to {odir}")
...
def plot_bolus():
    ...
    #plt.show()
    plt.savefig("inject_1.png", dpi=300)
    plt.close()
    breakpoint()
