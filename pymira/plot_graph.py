...
class Graph(object):
    ...
    #------------------------- added function----------------------------#    
    """def write_ply(graph, scalar_field_name, filename, cmap='jet', log_color=False, cmap_range=None):
    

        if not hasattr(graph, scalar_field_name):
            print(f"[WARN] Field '{scalar_field_name}' not in graph.")
            return

        scalar_field = getattr(graph, scalar_field_name)
        coords = graph.get_edgepoints()
        conns = graph.get_conns()
        nedgepoints = graph.get_nedgepoints()

        # Build geometry (simplified for clarity)
        lines = []
        colors = []

        offset = 0
        for i, num in enumerate(nedgepoints):
            indices = np.arange(offset, offset + num - 1)
            for j in indices:
                lines.append([j, j + 1])
                val = scalar_field[j]
                if log_color:
                    val = np.log10(val) if val > 0 else -5
                colors.append(val)
            offset += num

        colors = np.array(colors)
        if cmap_range:
            norm = plt.Normalize(vmin=cmap_range[0], vmax=cmap_range[1])
        else:
            norm = plt.Normalize(vmin=np.min(colors), vmax=np.max(colors))
        cmap = plt.cm.get_cmap(cmap)
        mapped_colors = cmap(norm(colors))[:, :3]

        line_set = o3d.geometry.LineSet()
        line_set.points = o3d.utility.Vector3dVector(coords)
        line_set.lines = o3d.utility.Vector2iVector(lines)
        line_set.colors = o3d.utility.Vector3dVector(mapped_colors)

        success = o3d.io.write_line_set(filename, line_set)
        if success:
            print(f"[SUCCESS] .ply saved to {filename}")
        else:
